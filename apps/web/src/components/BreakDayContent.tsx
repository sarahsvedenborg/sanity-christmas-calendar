       ...
       {/*  <div className="relative flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.45em] text-amber-600 dark:text-amber-200 md:text-sm">
        ...
        {isMemeOnly && featuredMeme ? (
          <div className="relative w-full max-w-4xl overflow-visible">
            <div
              className={`pointer-events-none absolute -left-10 top-10 z-20 rounded-sm bg-amber-200/90 px-5 py-2 text-sm font-black uppercase tracking-[0.4em] text-amber-900 shadow-lg transition-all duration-1000 ${
                hasLoaded
                  ? "animate-[lolTagDrop_0.8s_ease-out_0.2s_both]"
                  : "-rotate-12 -translate-y-20 opacity-0"
              }`}
            >
              lol
            </div>
            <div
              className={`relative overflow-visible rounded-[48px] border-4 border-white/70 bg-white/90 p-6 shadow-[0_30px_60px_rgba(2,6,23,0.35)] transition-all duration-1000 dark:border-amber-200/60 dark:bg-green-950/70 ${
                hasLoaded
                  ? "animate-[memeDrop_1.2s_cubic-bezier(0.68,-0.55,0.265,1.55)_both]"
                  : "-translate-y-32 rotate-[-15deg] scale-75 opacity-0"
              }`}
            >
